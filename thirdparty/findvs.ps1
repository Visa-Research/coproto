...
If ($vsWhere = Get-Command "vswhere.exe" -ErrorAction SilentlyContinue) {
 ...
 Else {
  $bbb = Test-Path "${env:ProgramFiles(x86)}\Microsoft Visual Studio\Installer\vswhere.exe"
  Write-Error "vswhere not found. Aborting. $bbb" -ErrorAction Stop
}
...
If ([string]::IsNullOrEmpty("$vsPath")) {
    $vsPath = &$vsWhere -version -latest -products * -requires Microsoft.Component.MSBuild -property installationPath -prerelease
    If ([string]::IsNullOrEmpty("$vsPath")) {
